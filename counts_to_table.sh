...
echo "intersect count files with peaks and sort"
...
# Chr(B73)  STop=POS(B73)   ID(NAMchr_NAMPOS)   REFal(B73)   ALTal(Paternal)   Count(B73)   Count(Pat) Peak(B73) Peak(Pat)  BF(BF (B73/(B73+Pat)), if both 0 print n.r.)
...
#Chr(B73)  STop=POS(B73) REFal(B73)   ALTal(Pat) ID(Patchr_PatPOS) Genotype (0/0 if B73, 1/1 if Pat) Count(B73)   Count(Pat) BF (n.r. if both 0)

#Add depth information, i.e. calculate number of reads B73 and Pat and change BF to "n.p. binding frequency" if none of the alleles has a peak at this position
